...
uint8 Communicate_ForAll(uint8 *pData)
{
    ...
    for(i = 1; i <= 6; i++) 	//MBUS通道4个。
    {
        ...
        if(lu16meternum > 0)
        {
            ...
            for(lu8RetryTimes = 0; lu8RetryTimes < 5; lu8RetryTimes++) //为保证群发成功率，群发2次。
            {
...
uint8 METER_MeterCommunicate_Direct_Ex(uint8 *pData, uint8 InLen, uint8 *pResBuf, uint8 *pOutLen, uint8 ForAllOrOne)
{
    ...
    if(ForAllOrOne == 0x0a)/*群发,给每个通道都发送下*/
    {
        for(i = 1; i < 7; i++)
        {
            METER_ChangeChannel(i);
...
uint8 METER_ReadMeterDataTiming(uint16 MeterSn, uint8 *EleBuf)
{
    ...
    // gDownCommDev485 = gMETER_Table[mf.ProtocolVer][3];  //靠通道号决定是否485总线即可。
...
ERROR:
    ...
    //begin:  设置温控面板显示热量值，并下发热量值。
    gu8ReadValveFail = 0;//抄阀控器前标记没失败。
    ...
    if(MeterNoBcdCheck(mf.ValveAddr) == TRUE)
    {
        ...
        if(ReadMeterSuccess) 		//抄读到有效数据才会将数据下发
        {
            //设置温控面板显示热量值。
    ...
    //end:  设置温控面板显示热量值，并下发热量值。

    p_ReadVavleTime = pTemp;  //预留抄阀控时间位置指针，阀控处理完后赋值。
    pTemp += 3;				  //时间占用3字节，秒、分、时。
    DataLen += 3;
    ...
    ReadDateTime(lu8ReadTime);
    memcpy(p_ReadVavleTime, lu8ReadTime, 3); //加入抄阀时间当前时间，只放时、分、秒。
...
/****************************************************************************************************
**	函 数  名 称: YILINVALVE_ReadMeterDataTiming
**	函 数  功 能: 集中器读取下挂亿林的数据及读取控制
**	输 入  参 数: uint16 MeterSn -- 表序号;
**	输 出  参 数: uint8 *EleBuf -- 接收到的数据帧回应;
**  返   回   值: NO_ERR, ERR_1 -- 没有接收到数据
**	备		  注: 填写下行读取数据的相关数据结构
...
uint8 YILINVALVE_ReadMeterDataTiming(uint16 MeterSn, uint8 *EleBuf)
{
    ...
    if(Err == NO_ERR)
    {
        if(ProtocoalInfo.DataBuf[4] & 0x2) //阀门无线故障
        {
            valve_t->data_valid = 0x0;
            valve_t->state |= 0x40;
        }
        ...
        valve_t->room_temperature = HexToBcd4bit(ProtocoalInfo.DataBuf[5]) * 0x100; //保存的时候后两位为小数
...
CJ188_Format METER_Data_To_CJ188Format(uint8 ProtocolVer, uint8 *DataBuf, uint8 len, uint8 *err)
{
    ...
    case ZHENYU_S_VER:
    {
        ...
        MeterData.Flow = Data.Flow;		 //瞬时流量	 -------两种力创表的不同点
        MeterData.FlowUnit = Data.FlowUnit;
        MeterData.AccumulateFlow = Data.AccumulateFlow1;	  //结算日累计流量
    ...
    case MITE_VER:
    {
        YITONGDA_Format Data;   //威海米特热表数据格式和天津亿通达一样。
        uint32 temp = 0;
        memcpy(&Data, DataBuf, sizeof(Data));
        temp = Data.CurrentCool;	//当前累计热量放到了当前冷量位置。
        temp = temp << 8;  //热量表对邋CJ188有改动，当前热量无小数位。
        MeterData.DailyHeat = temp;	//结算日热量,也赋成了总热量。
        MeterData.DailyHeatUnit = Data.CurrentCoolUnit;
        MeterData.CurrentHeat = temp;    //当前热量
        MeterData.CurrentHeatUnit = Data.CurrentCoolUnit;
        //MeterData.HeatPower = Data.HeatPower;	  //热表没有热功率
        MeterData.HeatPower = 0;
        MeterData.HeatPowerUnit = 0x17;

        temp = Data.Flow;
        temp = temp << 4; //薛城用亿通达热量表对邋CJ188有改动，流速3位小数位。
        MeterData.Flow = temp;	   //瞬时流量
        MeterData.FlowUnit = 0x35;	//单位强制立方米/小时。
        temp = Data.AccumulateFlow;
        temp = temp << 8;//累计流量2位小数位。
        MeterData.AccumulateFlow = temp;		//结算日累计流量
        MeterData.AccumulateFlowUnit = Data.AccumulateFlowUnit;
        memcpy(MeterData.WaterInTemp, Data.WaterInTemp, 3); 		 //进水温度
        memcpy(MeterData.WaterOutTemp, Data.WaterOutTemp, 3);		 //回水温度
