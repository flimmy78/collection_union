...
#define UART5_SEND_QUEUE_LENGTH				2072 			//给UART5发送数据队列分配的空间大小,单位：字节
#define UART5_REC_QUEUE_LENGTH			    2072 			//给UART5接收数据队列分配的空间大小,单位：字节
...
static OS_EVENT *UART5QMsg;									//中断到任务的消息队列
OS_EVENT *UART5RecQueSem; 							//与接收队列绑定的信号量
OS_EVENT *UART5SendQueSem;							//发送数据的信号量

OS_EVENT *UART5SendCompleteSem;            //485发送完成信号量
...
{
    QueueWrite_Int((void *)UART5SendQueue, Data); 					//数据入队列

    /*
    if(USART_GetFlagStatus(UART5, USART_FLAG_TXE) == SET)			//UART0发送保持寄存器空
    {
...
void UART5Send(uint8 *buf, uint32 n)
{
    ...
    //由于RS485半双工总线有切换时间要求，所以切换方向前延时，切换方向后也延时。
    OSTimeDly(5);
    RS485_SEND_DIR();
    OSTimeDly(5);
    ...
    OSSemPend(UART5SendCompleteSem, 0, &err); //等待接收数据
    OSTimeDly(5);
    RS485_RCV_DIR();
...
/*********************************************************************************************************
** 函数名称: UART5Getch
** 功能描述: 从UART5接收队列读一个字节，若队列内无数据则按OutTime做超时等待. 供上层调用
** 输　入: OutTime,等待超时时间
** 输　出: data 接收到的数据
** 全局变量: 无
** 调用模块:
...
uint8 UART5Getch(uint8 *data, uint16 OutTime)
{
    ...
    {
        OSSemPend(UART5RecQueSem, OutTime, &err); //等待接收数据
        FeedTaskDog();
        if(err != OS_ERR_NONE)
        {
            return err;
...
/*********************************************************************************************************
** 函数名称: TaskUART5Rec
** 功能描述: UART5接收任务，将收到的数据写入UART0接收队列，此任务应具有较高的优先级
** 输　入:
** 输　出:
** 全局变量:
** 调用模块:
...
void TaskUART5Rec(void *pdata)
{
    ...
    while(1)
    {
        ...
        QueueWrite((void *)UART5RecQueue, *pReadData); //将收到的数据写入队列
