...
uint16 gPARA_MeterChannel[METER_CHANNEL_NUM][METER_PER_CHANNEL_NUM];
//6个通道热计量表信息, MeterSn
uint8 gPARA_Meter_Failed[METER_CHANNEL_NUM][METER_PER_CHANNEL_NUM];
/*7个通道失败记录*/
uint16 gPARA_MeterChannelNum[METER_CHANNEL_NUM]; //每个通道表数量
uint16 gPARA_ConPanelChannel[METER_CHANNEL_NUM][METER_PER_CHANNEL_NUM];
//6个通道每个通道的温控面板信息, MeterSn
uint16 gPARA_ConPanelChannelNum[METER_CHANNEL_NUM]; //每个通道加载的温控面板数量
uint16 gPARA_ValveConChannel[METER_CHANNEL_NUM][METER_PER_CHANNEL_NUM];
//6个通道每个通道的阀门信息, MeterSn
uint16 gPARA_ValveConChannelNum[METER_CHANNEL_NUM]; //每个通道加载的阀门数量
/*end:yangfei added 2013-03-25 for add leye 485 meter*/
/************************************** 抄表时间节点 **********************************************/
uint16 gPARA_TimeNodes[TIME_NODE_NUM_MAX] =
{
    0
}; //当前抄表时间点，在内存中的备份
...
uint8 gu8ReadDataStore[METER_FRAME_LEN_MAX] =
{
    0
}; //用于存放刚刚抄过的表的信息，用于轮显或查询。

/********************MBUS短路保护用到的全局变量*********************************/
uint8 gu8MBusShortRising = 0;
//MBUS短路指示引脚是否出现上升沿，0-没有，1-出现上升沿。
uint8 gu8MBusShortFlag[METER_CHANNEL_NUM] =
{
    0
}; //标记对应MBUS通道通道短路,1-短路，0-不短路。
 ...
 **	备		  注:
 *****************************************************************************************************/
uint8 STORE_GetAddrOfMeterData(uint8 *pTime, uint16 MeterSn, uint32 *AddrOut,
                               uint8 Flag)
{
    ...
    if (TimeCheck(ReadTime))
        return ERR_1;
    //检验时间正确性

    MonthHEX = BcdToHex(ReadTime[ARRAY_MONTH]);

    AddrErrFlag = STORE_CalcTime(ReadTime, &TimeOffSet, Flag);
    //获取需读取的时间与基准时间的偏移量，以一个时间节点为单位
    if ((MonthHEX >= 0x0B) && (MonthHEX <= 0x03))
        //判断是否在采暖季，采暖季每个月的数据都存储
    {
        if (AddrErrFlag == NO_ERR)
        {
            if ((Flag & 0xF0) == FIND_DATA)
            {
                *AddrOut = (METERS_DATA_STORE_ADDR +  \
                            (TimeOffSet * METERS_DATA_LEN) + ((MeterSn + 1)
                                    * METER_FRAME_LEN_MAX));
            }
...
uint8 PARA_ReadMeterInfo(uint16 MeterSn, MeterFileType *pMeterFile)
{
    ...
    OS_ENTER_CRITICAL();
    memcpy((uint8 *) & (pMeterFile->MeterID), (uint8 *) &
           (gPARA_MeterFiles[MeterSn].MeterID), sizeof(MeterFileType));
    OS_EXIT_CRITICAL();
...
uint8 STORE_GetTimeNodeInfo(uint16 InTime, uint8 *pNodeIndex, int8 *pNodeOffset)
{
    ...
    uint16 TimeNodeCur[TIME_NODE_NUM_MAX] =
    {
        0x00
    };
    CPU_SR cpu_sr;
 ...
 **	备		  注:
 *****************************************************************************************************/
uint8 STORE_FindTimeNodeIndex(uint16 InTime, uint16 *pTimeNode,
                              uint8 *pNodeIndexOut)
{
    uint8 i = 0;
    uint8 FindFlag = 0x00;
    ...
    memcpy((uint8 *)TimeNodeCur, (uint8 *)pTimeNode, (TIME_NODE_NUM_MAX) *sizeof
           (uint16));
    ...
    for (i = 0; i < TIME_NODE_NUM_MAX; i++)
    {
        ...
        if ((i == (TIME_NODE_NUM_MAX - 1)) && (TimeNodeCur[i] <= InTime))
            //最后一个时间节点
        {
            FindFlag = i;
            break;
 ...
 *****************************************************************************************************/
uint8 FindTimeNodeIndex_Forward(uint16 InTime, DataStoreParaType *datastorePara,
                                uint8 *pNodeIndexOut, uint8 *pYesterday)
{
    uint8 i = 0;
    uint8 FindFlag = 0x00;
    uint8 CheckBinHour = 0x00;
    ...
    LOG_assert_param(pNodeIndexOut == NULL);
    memcpy((uint8 *)TimeNodeCur, (uint8 *)datastorePara->TimeNode,
           (TIME_NODE_NUM_MAX) *sizeof(uint16));
...
uint8 StoreYesterday(void)
{
    ...
    Err = SDSaveData(YesterdayPath, (uint8 *)gcYesterdayPath, sizeof
                     (gcYesterdayPath), 0);
    if (Err != NO_ERR)
    {
        LOG_WriteSysLog_Format(LOG_LEVEL_WARN, "WARNING: %s %d SDSaveData Error!",
...
uint8 STORE_InitTimeNodePara(uint8 *pDateTime, uint8 NodeIndex)
{
        ...
        //只允许每天一开始的时间节点数据有效
    {
        ...
        memcpy((uint8 *)InitPara.TimeNode, (uint8 *)gPARA_TimeNodes, sizeof
               (gPARA_TimeNodes));
        memcpy(SystemTime, gSystemTime, 6);
        OS_EXIT_CRITICAL();
        ...
        if (Err != NO_ERR)
        {
            LOG_WriteSysLog_Format(LOG_LEVEL_WARN,
                                   "WARNING: %s %d SDSaveData Error!", __FUNCTION__, __LINE__);
        }
        OSMutexPost(FlashMutex);
    }
#if 0
    ...
    STORE_GetAddrOfMeterData(pDateTime, 0, &LogicAddr, FIND_CURRENT |
                             FIND_PARA);

    OSMutexPend(FlashMutex, 0, &Err);
    Err = AppSdWrRdBytes(LogicAddr, sizeof(DataStoreParaType), (uint8 *)
                         &InitPara, SD_WR);
    OSMutexPost(FlashMutex);
#endif
    /*end   :yangfei added 2012-12-24 */
...
void GetVipDataFormSD(void)
{
    ...
    uint16 TimeNode[TIME_NODE_NUM_MAX + 2] =
    {
        0x00
    };
    ...
    if (ReplenishPara.WriteFlag == 0xAA55)
    {
        ...
        if ((gPARA_ReplenishPara.MeterInterval > 9999) ||
                (gPARA_ReplenishPara.MeterInterval < 100))
        ...
        if ((gPARA_ReplenishPara.ValveInterval > 9999) ||
                (gPARA_ReplenishPara.ValveInterval < 100))
    ...
    if (para.WriteFlag == 0xAA55)
    {
        ...
        ;
        memset((uint8 *)gPARA_ConPanelChannelNum, 0x00, sizeof
               (gPARA_ConPanelChannelNum));
        memset((uint8 *)gPARA_ValveConChannel, 0x00, sizeof(gPARA_ValveConChannel))
        ;
        memset((uint8 *)gPARA_ValveConChannelNum, 0x00, sizeof
               (gPARA_ValveConChannelNum));
        ...
        for (i = 0; i < para.MeterStoreNums; i++)
        {
            ...
            //	OSMutexPend (FlashMutex,0,&Err);
            Err = SDReadData("/METER_FILE_ADDR", &mf, sizeof(MeterFileType),
                             sizeof(MeterFileType) * i);
            //	OSMutexPost (FlashMutex);
            /*end  :yangfei modified 2012-12-27  */
            gPARA_MeterFiles[i] = mf;

            mfChannel = mf.ChannelIndex - 1;
            //集中器存储通道从0开始，而主站设置通道从1开始
            if (mfChannel > (METER_CHANNEL_NUM - 1))
                //防止因表参数信息错误，导致下面赋值时抄过数组范围。
            ...
            if (ChannelTemp >= METER_PER_CHANNEL_NUM)
                //防止数组超限。
        ...
        //防止卡错误时写数据死机
    {
        ...
        {
            //	OS_ENTER_CRITICAL();
            memcpy((uint8 *)gPARA_TimeNodes, (uint8 *) &TimeNode[1],
                   (TIME_NODE_NUM_MAX)*sizeof(uint16));
        ...
        {
            //	OS_ENTER_CRITICAL();
            memset((uint8 *)gPARA_TimeNodes, 0xFF, (TIME_NODE_NUM_MAX) *sizeof
                   (uint16));
            //	OS_EXIT_CRITICAL();
        ...
        Err = SDSaveData(NodePath, &gPARA_TimeNodesNum, sizeof(gPARA_TimeNodesNum)
                         , 8 + 2 * TIME_NODE_NUM_MAX);
        ...
        Err = SDReadData((const char *)"/LCD_CONTRAST_PARA", (uint8 *)
                         &NVLCDConraston, sizeof(LCDContrastPara), 0);
        if (Err != NO_ERR)
        {
    ...
    else
    {
        ...
        else
        {
            ...
            do
            {
                Err = SDReadData(NodePath, (uint8 *) &TimeNode[1],
                                 (TIME_NODE_NUM_MAX) * sizeof(uint16), 8);
