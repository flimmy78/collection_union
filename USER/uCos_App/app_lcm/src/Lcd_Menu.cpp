...
void MeterDataShow(uint16 nowNum, uint16 allNum, uint8 *DataBuf)
{
    ...
    {
        ShowChar(lu8line, lu8column, 0x45); //如果抄阀失败则显示EE.
        lu8column += 3;
    ...
    {
        lu16shi = lu8tmpArray[1] >> 4;    //室温处理显示
    ...
    {
        lu16tmp = lu8tmpArray[3] & 0x0f;  //对阀开度的一种特殊处理，显示百分比。
        lu16tmp = 9 - lu16tmp;
        lu16tmp = lu16tmp * 25; //实际是*100/4.
        if(lu16tmp == 100)  //只能显示2位数字。
            lu16tmp = 99;
        else if(lu16tmp > 100)  //大于100说明错误，显示0.
            lu16tmp = 0;

        lu16shi = lu16tmp / 10;    //室温处理显示
        lu16ge = lu16tmp % 10;
        ShowChar(lu8line, lu8column, (lu16shi + 0x30));
        lu8column += 3;
        ShowChar(lu8line, lu8column, (lu16ge + 0x30));
...
void ReadMeterStateShow(void)
{
    ...
    while(1)
    {
        ...
        if(1 == gu8ReadAllMeterFlag)    //如果正在全抄表，则轮显。
        {
            ...
            ShowMenuTop();
            ShowMenuBottom(1);
            MeterDataShow(gu16ReadMeterSuccessNum, gu16ReadMeterNum, gu8ReadDataStore); //调用显示。
        ...
        else 	//如果不在全抄表，则可查询。
        {
            if(lu8UpdataFlag)   //lu8UpdataFlag防止在不需要更新的时候读SD卡。
            {
                ...
                if(Res == NO_ERR)
                {
                    ...
                    if(Res == NO_ERR)
                        MeterDataShow(lu16DisplayNum + 1, lu16MeterNum, gu8ReadDataStore); //调用显示。+1是为了从1开始显示。
                    else
